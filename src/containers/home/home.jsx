  ...
  return (
    <div className={styles.container}>
        ...
        <div className={styles.videoCards}>{
          ...
          mapper.map((item, index) => <VideoCard key={index} title= {randomSentence({min:8, max:10}).slice(0, -1)} thumbnail={randomImage()} channelLogo={randomImage()} channel={randomSentence({min:1, max:3}).slice(0, -1)} views={`${randomNumber(10, 150)}${randomStringFromArray(characters)} views`} uploadedBefore={`${randomNumber(1, 7)} ${randomStringFromArray(time)} ago`}/>)
        }
        </div>
    </div>
  )
